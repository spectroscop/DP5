...
def SetupNMRCalcs(Isomers, settings):
    ...
    for iso in Isomers:
        ...
        for num in range(0, len(conformers)):
            filename = iso.BaseName + 'nmr-nwinp' + str(num + 1).zfill(3)
...
def SetupECalcs(Isomers, settings):
    ...
    for iso in Isomers:
        ...
        if iso.DFTConformers == []:
            ...
            conformers = iso.DFTConformers
        for num in range(0, len(conformers)):
            filename = iso.BaseName + 'singl-nwinp' + str(num + 1).zfill(3)
            if os.path.exists(filename + '.nwo'):
                if IsNWChemCompleted(filename + '.nwo'):
...
def SetupOptCalcs(Isomers, settings):
    ...
    for iso in Isomers:
        ...
        if iso.DFTConformers == []:
            ...
            conformers = iso.DFTConformers
        for num in range(0, len(conformers)):
            filename = iso.BaseName + 'opt-nwinp' + str(num + 1).zfill(3)
            if os.path.exists(filename + '.nwo'):
                if IsNWChemCompleted(filename + '.nwo'):
...
#        outp = subprocess.check_output(NWChemPrefix + ' ' + f + ' > ' + f[:-2] + 'nwo', shell=True)
        print('mpirun -np 2 ' + NWChemPrefix + ' ' + f + ' > ' + f[:-2] + 'nwo')
        outp = subprocess.check_output('mpirun -np 2 ' + NWChemPrefix + ' ' + f + ' > ' + f[:-2] + 'nwo', shell=True)
        NCompleted += 1
        print("NWChem job " + str(NCompleted) + " of " + str(len(NWJobs)) + " completed.")
...
#        suffix += 'dft\n  xc b3lyp\n  mult 1\nend\n\n'
#    elif (settings.nFunctional).lower() == 'm062x' or (settings.nFunctional).lower() == 'm06-2x':
#        suffix +=  'dft\n  xc m06-2x\n  mult 1\nend\n\n'
...
def ReadShieldings(Isomers):
    ...
    for iso in Isomers:
        ...
        for NWOutpFile in iso.NMROutputFiles:
            print("NWChem.py, ReadShieldings: NMR DFT output " + NWOutpFile +
                  " files found, NMR data to be read.")
            nwfile = open(NWOutpFile, 'r')
            NWOutp = nwfile.readlines()
            nwfile.close()

#            print('  lenght  ', len(NWOutp))
            index = 0
...
def ReadGeometries(Isomers, settings):
    ...
    if ('o' in settings.Workflow):
        ...
        for iso in Isomers:
            ...
            if len(iso.NMROutputFiles) < 1:
                print("NWChem.py, ReadGeometries: No NMR geometry optimisation output" +
                      " files found, geometries could not be read. Quitting.")
                quit()
