...
def process_proton(NMR_file, settings, datatype):
    ...
    gradient_peaks, gradient_regions, gradient_groups, std = gradient_peak_picking(total_spectral_ydata, corr_distance, uc, noise_std, peak_regions)
    import time
    start = time.time()
    ...
    sim_regions, full_sim_data = simulate_regions(total_params, peak_regions, grouped_peaks, total_spectral_ydata, spectral_xdata_ppm)

    peak_regions, grouped_peaks, sim_regions, integral_sum, cummulative_vectors, integrals, number_of_protons_structure, optimum_proton_number, total_integral = find_integrals(settings.InputFiles[0], peak_regions, grouped_peaks, sim_regions, picked_peaks, total_params, total_spectral_ydata, solvent_region_ind)
...
#    total_spectral_ydata = ng.proc_base.fft_positive(total_spectral_ydata)  # Fourier transform
    total_spectral_ydata = ng.proc_base.zf_double(total_spectral_ydata, 3)  # zero filling once
    total_spectral_ydata = ng.proc_base.fft_positive(total_spectral_ydata)  # Fourier transform
...
#    tydata = ACMEWLRhybrid(total_spectral_ydata, corr_distance)
    tydata = acme(total_spectral_ydata, corr_distance)
#    print ("total_spectral_ydata ", len(total_spectral_ydata))
#    quit ()
...
def acme(y, corr_distance):
    ...
    out = Minimizer(acmescore, params, fcn_args=(np.imag(y), np.real(y), phase_order))
    results = out.minimize()
    p = results.params
    p.pretty_print()
...
def ACMEWLRhybrid(y, corr_distance):
    ...
    for region in peak_regions:
        ...
        out = Minimizer(residual_function, params, fcn_args=(np.imag(y[region]), np.real(y[region])))
...
#    quit ()
#    tmp = np.lib.stride_tricks.as_strided(a, shape=shape, strides=strides)
#    return tmp
...
#    print ("peak_regions ", peak_regions)
    ...
    for ind1, speak_ppm in enumerate(exp_ppm):
        ...
        # do optimisation problem with projected peaks
            ...
            for peak in picked_peaks_ppm[srange]:
                ...
                fit_s_peaks, amp_vector, fit_s_y = new_first_order_peak(peak, Jv[ind1], np.arange(len(x_data)), 0.1, uc, 1)
