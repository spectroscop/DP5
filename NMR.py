...
#--------------------
    def __init__(self,settings):
        ...
        self.Type = 'desc'          # fid or desc, depending on whether the description or raw data used
...
#            gdir = settings.OutputFolder /  "Graphs"
        ...
        fname = os.path.splitext(os.path.basename(settings.InputFiles[0]))[0]
...
#            self.protondata = pickle.load(open(pdir / settings.InputFiles[0] / "protondata", "rb"))
        if Path(pdir +  "/" + fname + "/protondata").exists():
            self.protondata = pickle.load(open(pdir +  "/" + fname + "/protondata", "rb"))
            self.Hshifts = self.protondata["exppeaks"]
        else:
            protondata = {}
            protondata["exppeaks"], protondata["xdata"], protondata["ydata"], protondata["integrals"], \
              protondata["peakregions"], protondata["centres"], protondata["cummulativevectors"], \
              protondata["integralsum"], protondata["picked_peaks"], protondata["params"], \
              protondata["sim_regions"] = process_proton(NMR_file, settings, self.Type)
#            pickle.dump(protondata, Path(pdir / settings.InputFiles[0] / "protondata").open(mode =  "wb+"))
            pickle.dump(protondata, Path(pdir +  "/" + fname + "/protondata").open(mode =  "wb+"))
...
#            self.carbondata = pickle.load(open(pdir / settings.InputFiles[0] / "carbondata", "rb"))
        if Path(pdir +  "/" + fname + "/carbondata").exists():
            self.carbondata = pickle.load(open(pdir +  "/" + fname + "/carbondata", "rb"))
            self.Cshifts = self.carbondata["exppeaks"]
        else:
            carbondata = {}
            carbondata["ydata"], carbondata["xdata"], carbondata["corrdistance"], carbondata["uc"], \
            carbondata["exppeaks"], carbondata["simulated_ydata"], carbondata["removed"] = process_carbon(NMR_file, settings,self.Type)
#            pickle.dump(carbondata, Path(pdir / settings.InputFiles[0] / "carbondata").open(mode =  "wb+"))
            pickle.dump(carbondata, Path(pdir +  "/" + fname + "/carbondata").open(mode =  "wb+"))
...
#        print ('BoltzmannShieldings: ', BoltzmannShieldings)
    return Isomers
...
def GetTMSConstants(settings):
    ...
    print("Unscaled shifts might be inaccurate, use of unscaled models is not recommended.")
