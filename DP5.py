...
class DP5data:
    ...
    def __init__(self,ScriptPath,Atoms):
        ...
        #self.folded_unscaled_errors = pickle.load(open(ScriptPath / "data/folded_unscaled_errors.p", "rb"))
...
def kde_probs(Isomers,dp5Data,sigma):
    def kde_probfunction(conf_shifts, conf_reps):
        ...
        for e, s_e, r in zip(errors, scaled_errors, conf_reps):
            ...
            K_sim = get_atomic_kernels(np.array([r]), dp5Data.atomic_reps, [sigma], cut_distance=c_distance)[0][0]
...
#        print ("dp5Data.AtomReps[iso] ", dp5Data.AtomReps[iso])
        ...
        for conf_shifts , conf_reps in zip(dp5Data.ConfCshifts[iso],dp5Data.AtomReps[iso] ) :
            res[ind1] = pool.apply_async(kde_probfunction, [conf_shifts,conf_reps])
            ind1 += 1
...
#    OutputFolder = Path(Settings.OutputFolder)
    OutputFolder = Settings.OutputFolder + "/dp5/"
    # extract calculated and experimental shifts and add to dp5Data instance Carbon
    ...
    if dp5Data.Atom_number < 86:
        for iso in Isomers:
            #open new xyz file
            InputFile = os.path.splitext(os.path.basename(iso.InputFile))
...
#                xyz_file = open(str(OutputFolder / "dp5" /InputFile.stem) + "_" +str(i).zfill(3) + ".xyz", "w")
                xyz_file = open(OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + ".xyz", "w")
...
#                dp5Data.Compounds.append(qml.Compound(xyz = str(Settings.OutputFolder +"/dp5/" + InputFile.stem) +"_"+ str(i).zfill(3) + ".xyz"))
                dp5Data.Compounds.append(qml.Compound(xyz = OutputFolder + InputFile[0] +"_"+ str(i).zfill(3) + ".xyz"))
    ...
    #otherwise we need to fragment the molecule to radius of 3
        ...
        for iso in Isomers:
            ...
            #find conformer with the lowest energy
            dp5Data.AtomReps.append([])
            print (OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + ".xyz")
            quit ()
            for i,geom in enumerate(iso.DFTConformers):
#                xyz_file = open(str(OutputFolder / "dp5" /InputFile.stem) + "_" +str(i).zfill(3) + ".xyz", "w")
                xyz_file = open(OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + ".xyz", "w")
...
#                mol_fragments(m,str(OutputFolder / "dp5" /InputFile.stem) + "_" +str(i).zfill(3) + "_fragments")
                os.mkdir(OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + "_fragments")
                mol_fragments(m, OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + "_fragments")
                conf_rep = []
...
#                    c = qml.Compound(xyz=str(OutputFolder / "dp5" /InputFile.stem) + "_" +str(i).zfill(3) + "_fragments/" + xyz_frag)
                for xyz_frag in sorted(os.listdir(OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + "_fragments")):
                    c = qml.Compound(xyz=OutputFolder + InputFile[0] + "_" +str(i).zfill(3) + "_fragments/" + xyz_frag)
...
#    print ('ScaledAtomProbs', dp5Data.ScaledAtomProbs)
...
# <stdin>:1: DeprecationWarning: Please use `gaussian_kde` from the `scipy.stats` namespace, the `scipy.stats.kde` namespace is deprecated.
...
#    correct_kde = pickle.load(open(Path(Settings.ScriptDir) / "data/c_w_kde_mean_s_0.025.p" ,"rb"))
    incorrect_kde = pickle.load(open(Settings.ScriptDir + "/data/i_w_kde_mean_s_0.025.p" ,"rb"))
    correct_kde   = pickle.load(open(Settings.ScriptDir + "/data/c_w_kde_mean_s_0.025.p" ,"rb"))
    i = 0
...
#        dp5Data.BScaledAtomProbs[i] = [ 1 - float(incorrect_kde.pdf(x) / (incorrect_kde.pdf(x) + correct_kde.pdf(x))) for x in scaled  ]
        dp5Data.BScaledAtomProbs[i] = [ 1 - float(incorrect_kde.pdf(scaled) / (incorrect_kde.pdf(scaled) + correct_kde.pdf(scaled))) ]
#        print ('scaled ', scaled)
        i += 1
#        quit ()
...
def mol_fragments(mole,outfile):
    ...
    for atom, exp,dft,diff in zip(OBMolAtomIter(mole)):
        ...
        for atom in OBMolAtomIter(new_mol):
            f.write(atom.GetType()[0] + " " + str(atom.GetX()) + " " + str(atom.GetY()) + " " + str(atom.GetZ()) + "\n")
            i+=1
        f.close()
        c += 1
